...
LS_TEACHER_PHOTOS = {
    ...
    "Олжабай Дамир": "olzhabai-damir.jpg",
    "Олжабаев Дамир": "olzhabai-damir.jpg",
...
# себепті синхрондау БІР ҮЛКЕН сұранысқа емес, әрқайсысы бірнеше топты
# ғана өңдейтін көптеген ШАҒЫН қадамдарға (/start + қайта-қайта /step)
# бөлінген. Ескі 3/6 мәні тым сақтық үшін төмен болатын (толық
# синхрондау >100с созылатын) — енді 0-оқушылы топтар сүзілгеннен кейін
# (ағымда ~47 топ, 84 емес) 24-ке дейінгі параллельдікте де Juz40 API
# ешбір қатесіз (0 errors) жауап беретіні нақты өлшеумен расталды
# (114с → ~20-25с). Сақтық үшін 24-тен төмен, 10-мен тоқтадық.
JUZ40_IMPORT_WORKERS = 10
JUZ40_STEP_CHUNK_SIZE = 10
...
# JUZ40_QUESTION_CHUNK_SIZE әдейі 1 болып қалады (бір қадамда БІР ғана топ)
# — өткенде бірнеше АУЫР (PDF-ті көп) топты бір мезгілде өңдеу 5 минуттық
# аспаудың себебі болған. Бірақ топтың ІШІНДЕГІ оқушы параллельдігі
# (WORKERS/BATCH) нақты production деректерімен (ТАРИХ-01, ТАРИХ-11, 75-94
# оқушылы топтар қоса) өлшенді: 20/40-та да Juz40 API әрдайым қатесіз
# жауап береді (94 оқушылы топ ~35с-та, PDF кэші жылынған соң 90 оқушылы
# топ 12с-та аяқталды) — ескі 4/8 тым сақтық үшін төмен болған.
JUZ40_QUESTION_CHUNK_SIZE = 1
JUZ40_QUESTION_STUDENT_WORKERS = 8
JUZ40_QUESTION_STUDENT_BATCH = 16
JUZ40_QUESTION_STEP_BUDGET_SECONDS = 15
...
def import_juz40_questions_start(week_id):
    ...
    # Осы аптаның ЕҢ СОҢҒЫ job-ын қараймыз.
    existing_job = conn.execute(
        "SELECT * FROM juz40_question_jobs WHERE week_id = ? ORDER BY id DESC LIMIT 1",
        (week_id,),
    ).fetchone()
    if existing_job:
        pending = json.loads(existing_job["pending_json"] or "[]")
        # АЯҚТАЛМАҒАН job бар ма (browser жабылып/интернет үзіліп қалған) —
        # солай болса, ЖАҢАДАН бастамай СОНЫ жалғастырамыз: бұрын сақталған
        # сұрақ-жауап деректерін тазаламаймыз, топтар тізімін Juz40-тан
        # қайта алмаймыз.
        if existing_job["status"] == "running" and pending:
            return jsonify({
                "ok": True, "job_id": existing_job["id"], "total": existing_job["total_groups"],
            })
        # Job толық АЯҚТАЛҒАН болса — Juz40-тан бәрін қайта оқып, қайта
        # тексеріп жатпаймыз (бұл процесс баяу, бірнеше минутқа созылады).
        # Жаңа деректер керек болса, куратор жаңа лекция/апта қосқанда ол
        # жаңа week_id-мен келеді — сол себепті осы аптаға қайта басу тек
        # артық уақыт жоғалтады.
        if existing_job["status"] == "done":
            return jsonify({
                "ok": True, "done": True, "already_done": True,
                "job_id": existing_job["id"], "total": existing_job["total_groups"],
                "processed": existing_job["processed_count"],
                "inserted": existing_job["inserted_count"],
                "failed": existing_job["failed_count"],
            })

    # Батырманы ЖАҢАДАН бассаңыз (алдыңғы job мүлде болмаған не қатемен
    # аяқталған), осы аптаның алдыңғы сұрақ-жауап деректерін алдын ала
