...
def index():
    conn = get_db()
    weeks = conn.execute("SELECT * FROM weeks ORDER BY id DESC").fetchall()
    weeks_with_counts = []
    for w in weeks:
    ...
    return render_template("index.html", weeks=weeks_with_counts)
...
def create_week():
    ...
    week_id = conn.execute(
        "INSERT INTO weeks (title, start_date, end_date, passing_score) VALUES (?, ?, ?, ?) RETURNING id",
        (title, start_date or None, end_date or None, float(passing_score) if passing_score else None),
    ).fetchone()["id"]
    conn.commit()
